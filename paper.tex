...
\begin{enumerate}
  ...
  \item same\_srv\_rate: the percentage of connections to the same service. \mycomment{Have example of service}
\end{enumerate}
...
%A \emph{false positive} (FP) happens when an IDS incorrectly identifies normal activity as being an attack. A \emph{false negative} (FN) happens when an IDS fails to identify harmful activity. A \emph{true positive} (TP) happens when an IDS correctly identifies activities to be attacks. A \emph{true negative} (TN) happens when an IDS correctly identifies activities to be normal.~\cite{ids}
...
\begin{table}
\caption{Results for KDD99 with Certain Attacks~\cite{6559603}}
\vspace{0.20cm}
\begin{tabular}{cccccc}
...
\subsection{Algorithm Overview}
The authors of~\cite{DBLP:journals/corr/abs-1204-1336} used a genetic algorithm to develop an IDS.
...
%& & Predicted & & \\
%Actual & Normal & Probe & DoS & \% Correct \\ \cline{2-5}
%Normal & 42138 & 1421 & 15835 & 69.5\\
%Probe & 398 & 2963 & 654 & 71.1   \\
%Dos & 921 & 432 & 228489 & 99.4\\
%&&&& \\
%\% Correct &  76.9 & 54.7 & 92.0 &\\
%\end{tabular}
%\center
%\label{tab:genAlgResults}
%\end{table}
